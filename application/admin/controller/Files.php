...
class Files extends Backend
{
	...
	{
		$act = input('act', '');
		$uploadurl = url('hhadmin/ajax/upload','','.html',true);
	...
	public function editorUpload()
    {
        ...
        try {
            ...
            $savekey = $upload->getSavekey('/uploads/editor/'.$dir."/{year}{mon}{day}/{filemd5}{.suffix}");
            $attachment = $upload->upload($savekey);
        } catch (UploadException $e) {
            $this->error($e->getMessage());
        }

        $ossSwitch = Config::get('site.switch');
        if($ossSwitch){
            $res = controller('admin/oss')->uploads($attachment->url);
            if ( !$res['success'] ) {
                return json(['error' => 1, 'message' => $res['msg'], 'absurl' =>'','url'=>'']);
            }else{
                return json(['error' => 0, 'message' => '', 'absurl' => $res['file'],'url'=>$res['file']]);
            }
        }

        $absfile = ROOT_PATH . 'public';
        return json(['error' => 0, 'message' => '', 'absurl' => $absfile . $attachment->url,'url'=>cdnurl($attachment->url,true)]);
    }
     ...
     * 雪殇地图在线制作
     */
	public function hhmap()
	{
		return $this->fetch('', ['title' => '雪殇地图在线制作']);
	}
	...
	public function maps()
    {
        $map          = array();
        $data         = input('get.');
        $map['name']  = isset($data['name'])  ? self::zystr($data['name'])  : Config::get("site.company_name");
        $map['tel']   = isset($data['tel'])   ? self::zystr($data['tel'])   : Config::get("site.company_phone");
        $map['email'] = isset($data['email']) ? self::zystr($data['email']) : Config::get("site.company_email");
        $map['add']   = isset($data['add'])   ? self::zystr($data['add'])   : Config::get("site.company_address");
        $map['jcode'] = isset($data['jcode']) ? self::zystr($data['jcode']) : '0';
        $map['wcode'] = isset($data['wcode']) ? self::zystr($data['wcode']) : '0';
        $map['type']  = isset($data['type'])  ? (int)$data['type']  : 1;
        if ( $map['type'] > 2 || !$map['type'] ) $map['type'] = 2;
        return $this->fetch('',['map'=>$map]);
    }

    private function zystr( $str = '' )
    {
        return $str!='' ? addslashes(htmlspecialchars(trim($str))) : '';
    }
	...
	public function wxarticle()
	{
		...
		if ($html != '') {
			$ql      = new \ql\QueryList;
			$data    = $ql::Query($html, array('titleTag' => array('#activity-name', 'text'), 'author' => array('#js_name', 'text'), 'contentWx' => array('#js_content', 'html'), 'imageWx' => array('img', 'data-src')))->data;
			...
			foreach ($data as $key => $val) {
				if ($val['imageWx'] != '') {
					...
					if ($pic != '') {
						$pic = root() . '/uploads/editor/image/' . $pic;
	...
	{
		$php_path  = ROOT_PATH. 'public/';
		$php_url   = root() .'/uploads/editor/';
		...
		if ($dir_name !== '') {
            if (!file_exists($root_path.$dir_name)) {
                mkdir($root_path.$dir_name);
            }
			$root_path .= $dir_name . "/";
			$root_url .= $dir_name . "/";
		}
	...
	public function uploadword()
	{
		$exts    	= 'doc,docx';
		$maxSize 	= 1024 * 1024 * 5;
		$movepath   = ROOT_PATH.'public/uploads/files/';
		$uploadname = 'file';
		$validate   = array('size' => $maxSize, 'ext' => $exts);
		$dpath      = date('Ymd');
		$file       = request()->file($uploadname);
		if ( !$file ) return json(array('msg' => '无效的文件数据..','success'=>0));
		$info       = $file->rule('uniqid')->validate($validate)->move($movepath.$dpath);
		if ( $info ) {
			$file 	   = $movepath.$dpath.'/'.$info->getSaveName();
			unset($info);
            $fileType = pathinfo($file, PATHINFO_EXTENSION)=='docx'?'Word2007':'MsDoc';
			$wordobj   = IOFactory::createWriter(IOFactory::load($file,$fileType), "HTML");
			$html	   = $wordobj->getContent();
            //处理word文档中的图片
            $imgArr = $this->getPatternMatchImages($html);
            if($imgArr){
                if(!is_array($imgArr)){
                    $imgs = $this->base64ToImg(['base64str'=>$imgArr]);
                    $img = isset($imgs['picurl'])?$imgs['picurl']:$imgArr;
                    $html = str_replace($imgArr,$img,$html);
                }else{
                    foreach ($imgArr as $key=>$value){
                        $imgs = $this->base64ToImg(['base64str'=>$value]);
                        $img = isset($imgs['picurl'])?$imgs['picurl']:$value;
                        $html = str_replace($value,$img,$html);
                    }
                }
            }

			$search    = ["'<script[^>]*?>.*?</script>'si","'<head[^>]*?>.*?</head>'si","'<style[^>]*?>.*?</style>'si","'<!--[/!]*?[^<>]*?>'si","'([rn])[s]+'"];
			...
			$html      = str_ireplace('</html>','',$html);
			@unlink($file);
			return json(['msg'=>'','html'=>$html,'success'=>1]);
		} else {
			return json(['msg' => $file->getError(),'success'=>0]);
		}
	}
    ...
    public function getPatternMatchImages($contentStr = ""){
        $imgSrcArr = [];
        //将富文本字符串中的 img 标签以及 img的src 属性进行匹配
        $pattern= '/<img\b.+\bsrc\b\s*=\s*[\'\"]([^\'\"]*)[\'\"]/i';
        preg_match_all($pattern,$contentStr,$imgSrcArr);
        if(isset($imgSrcArr[1])){
            $imgSrcArr = $imgSrcArr[1];
        }else{
            $imgSrcArr = [];
        }
        return $imgSrcArr;
    ...
    {
        $text = '雪殇网络核心团队组建于2010年，于该年注册上海雪殇网络科技有限公司；上海雪殇网络科技有限公司 是上海专业的网络互动项目开发以及顾问服务提供商，我们的服务范围包括：APP开发、微信开发、小程序开发、网站建设、UI设计、云服务器、整合营销一条龙服务。';
        $tpl = [
            1 => [
			...
			<fieldset style="margin: 20px 0px 0px 10px; padding: 0px; border: 0px; max-width: 100%; color: rgb(62, 62, 62); font-family: 微软雅黑; line-height: 25px; white-space: normal; word-wrap: break-word !important; box-sizing: border-box !important; background-color: rgb(255, 255, 255);">
                ...
                ['html' => '<section class="hhtpl-e"><section style="display:flex;justify-content:center;"><p style="margin:0px;text-align:center;color:#fff;height:35px;line-height:35px;border-radius:5px;background-color:#36aeea;font-size:18px;padding:0px 15px;text-align:center;">文章推荐</p>
                ...
                ['html' => '<section class="hhtpl-e"><section><section style="display:flex;align-items:center;margin-bottom:8px;"><p style="width:100px;text-align:right;font-size:16px;margin:0px;flex-shrink:0;">第一阶段</p>
                        <section style="width:40px;margin:0px 10px;flex-shrink:0;"></section><p style="text-align:center;font-size:16px;margin:0px;flex-shrink:1;line-height:35px;color:#fff;background-color:#ffd174;border-radius:0px 0px 10px 0px;min-width:150px;padding:0px 15px;">销售额30万</p>
                        ...
                        <section style="width:40px;margin:0px 10px;flex-shrink:0;"></section><p style="text-align:center;font-size:16px;margin:0px;flex-shrink:1;line-height:35px;color:#fff;background-color:#ffd174;border-radius:0px 0px 10px 0px;min-width:150px;padding:0px 15px;">销售额60万</p>
                        ...
                        <section style="width:40px;margin:0px 10px;flex-shrink:0;"></section><p style="text-align:center;font-size:16px;margin:0px;flex-shrink:1;line-height:35px;color:#fff;background-color:#ffd174;border-radius:0px 0px 10px 0px;min-width:150px;padding:0px 15px;">销售额100万</p>
                    </section></section></section>'],
                ['html' => '<section class="hhtpl-e"><section style="max-width: 360px !important;margin: 0px auto;text-align: center;"><section style="display: flex;margin: 0px auto;text-align: center;justify-content: space-around;"><section style="background-image: url(http://kol-statics.oss-cn-beijing.aliyuncs.com/editor/170032769e762551acfcd35e88ab4f6ac358e7.png);background-repeat: no-repeat;background-size: 100% auto;width: 140px;height:65px;line-height: 65px;box-sizing: border-box;padding: 5px;"><section style="border:1px solid #fff;display: flex;height: 55px;"><p style="color: #fff;font-size: 23px;margin: 0px;line-height: 50px;"><span style="font-size: 14px;">￥</span>50</p>
...
<br><p>our foot steps are moving towards the end of own chosen target.</p></section></section>'],
					...
					<tbody style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;">
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height: 30px;">
							...
							<td style="word-break: break-all; max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc;border-top: 1px solid #ddd">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">HH-QZW-2022-0307-1622</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height: 30px;">
							...
							<td style="word-break: break-all; max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">雪殇CMS系统</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height: 30px;">
							...
							<td style="word-break: break-all; max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">外国语学院B标4号</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height:30px;">
							...
							<td style="word-break: break-all; max-width: 100%;word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">普通大屏显示器</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height: 30px;">
							...
							<td style="word-break: break-all; max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">XSQ-Z48</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height:30px;">
							...
							<td style="word-break: break-all; max-width: 100%;  word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">2022-03-07</span>
							</td>
						...
						<tr style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;line-height: 30px;">
							...
							<td style="word-break: break-all; max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important;border-bottom: 1px solid #ddd;padding-left:10px;background: #fcfcfc">
								<span style="max-width: 100%; word-wrap: break-word !important; box-sizing: border-box !important; font-size: 10pt;">上海国安检测有限公司</span>
							</td>
						...
						<table style="border-collapse:collapse;">
							<tbody>
								<tr>
									...
									<td style="border: 1px solid #7ecef4;width: 350px;height : 32px;text-align: center;">68<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">糖醋里脊</td>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">45<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
									<td style="border: 1px solid #7ecef4;width:100px;height : 32px;text-align: center;">鱼香肉丝</td>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">30<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
									<td style="border: 1px solid #7ecef4;width: 6em;height : 32px;text-align: center;">地三鲜</td>
									<td style="border: 1px solid #7ecef4;width: 6em;height :32px;text-align: center;">58<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">凉拌土豆丝</td>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">18<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">22<span style="font-size: 12px;color:#888;"> 元/份</span></td>
								</tr>
								<tr>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">可乐</td>
									<td style="border: 1px solid #7ecef4;width: 100px;height : 32px;text-align: center;">5<span style="font-size: 12px;color:#888;"> 元/杯</span></td>
								</tr>
								<tr>
				...
				<section style="margin: 10px auto; padding: 0px; max-width: 100%; box-sizing: border-box; border: 1px solid rgb(197, 200, 204); text-align: center; border-radius: 5px; box-shadow: rgb(255, 255, 255) 0px 1px 0px, rgb(197, 200, 204) 0px 2px 0px, rgb(255, 255, 255) 0px 3px 0px, rgb(197, 200, 204) 0px 4px 0px; word-wrap: break-word !important;">
					...
					<section style="margin: 40px 10px; padding: 0px; max-width: 100%; box-sizing: border-box; word-wrap: break-word !important;">
						...
						<p style="margin:0 auto; padding: 10px; max-width: 100%; box-sizing: border-box; clear: both; text-align: left; font-size:14px;word-wrap: break-word !important;height: 40px;line-height: 40px;overflow: hidden;">
							...
							<span style="margin-left: 10px;">地点：上海普陀交通路4703弄李子园商务区6号楼705、706室</span>
						</p>
						<p style="margin:0 auto; padding: 10px; max-width: 100%; box-sizing: border-box; clear: both; text-align: left; font-size:14px;word-wrap: break-word !important;height: 40px;line-height: 40px;overflow: hidden;">
							<span class="icon-Wei_UI_user" style="color:#999;font-size:16px;"></span>
							<span style="margin-left: 10px;">人员：蒋婷，雪霁初晴，张小呆</span>
						</p>
					</section>
				</section>
			</section>
		...
		<section style="margin: 0px; padding: 0px; display: inline-block; width: 100%; box-sizing: border-box;" data-width="100%"><section style="margin: 10px;padding: 0px 0px 15px;box-sizing: border-box;background-color: rgb(255, 255, 255);border-radius: 10px;"><section style="margin: 0px; padding: 0px; display: inline-block; width: 100%; box-sizing: border-box;" data-width="100%"><section style="margin-left: 5px; padding: 0px; width: 10px; height: 10px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section><section style="margin-right: 5px; padding: 0px; width: 10px; height: 10px; border-radius: 50%; float: right; clear: none; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section></section><section style="margin: 0px; padding: 0px; text-align: center; box-sizing: border-box;"><section style="margin: 0px; padding: 0px; display: inline-block; box-sizing: border-box;"><section style="margin-top: 10px; padding: 0px; box-sizing: border-box; float: left;"><section style="margin: 0px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section><section style="margin: 0px 3px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section><section style="margin: 0px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section></section><section style="margin: 0px 5px; padding: 0px; box-sizing: border-box; float: left; color: rgb(239, 112, 96); font-weight: bold;"><section style="margin: 0px; padding: 0px; color: inherit; float: left;transform: rotate(-10deg);-webkit-transform: rotate(-10deg);-moz-transform: rotate(-10deg);-o-transform: rotate(-10deg);"><span style="margin: 0px; font-size: 20px; color: inherit;">活</span></section><section style="margin: 0px; padding: 0px; color: inherit; float: left;transform: rotate(10deg);-webkit-transform: rotate(10deg);-moz-transform: rotate(10deg);-o-transform: rotate(10deg);"><span style="margin: 0px; font-size: 20px; color: inherit;">动</span></section><section style="margin: 0px; padding: 0px; color: inherit; float: left;transform: rotate(-15deg);-webkit-transform: rotate(-15deg);-moz-transform: rotate(-15deg);-o-transform: rotate(-15deg);"><span style="margin: 0px; font-size: 20px; color: inherit;">细</span></section><section style="margin: 0px; padding: 0px; color: inherit; float: left;transform: rotate(5deg);-webkit-transform: rotate(5deg);-moz-transform: rotate(5deg);-o-transform: rotate(5deg);"><span style="margin: 0px; font-size: 20px; color: inherit;">则</span></section><section style="margin: 0px; padding: 0px; color: rgb(255, 255, 255); width: 28px; height: 28px; line-height: 28px; border-radius: 50%; float: left; background-color: rgb(239, 112, 96);transform: rotate(5deg);-webkit-transform: rotate(5deg);-moz-transform: rotate(5deg);-o-transform: rotate(5deg);" data-width="28px"><span style="margin: 0px; font-size: 20px; color: #FFFFFF;">&amp;</span></section><section style="margin: 0px; padding: 0px; color: inherit; float: left;transform: rotate(5deg);-webkit-transform: rotate(5deg);-moz-transform: rotate(5deg);-o-transform: rotate(5deg);"><span style="margin: 0px; font-size: 18px; color: inherit;">FAQ</span></section></section><section style="margin-top: 10px; padding: 0px; box-sizing: border-box; float: left;"><section style="margin: 0px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section><section style="margin: 0px 3px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section><section style="margin: 0px; padding: 0px; width: 5px; height: 5px; border-radius: 50%; float: left; color: rgb(255, 255, 255); box-sizing: border-box; background-color: rgb(239, 112, 96);"></section></section></section></section><section style="margin: 10px; padding: 0px; box-sizing: border-box;color: #000000;"><section class="135brush"><p>' . $text . '</p><p>——雪殇</p></section></section></section></section></section>'],
			...
			<section style="border: 1px solid #e2e2e2;line-height: 1.6em;">
				...
				<section style="margin-top: 1.5em; text-align: left;">
					...
					<section style="display: inline-block; width: 60%; padding: 0.2em; margin-left: 0.5em; font-size: 1em; font-style: normal; color: inherit;" data-width="60%">活动时间：2022年3月09日 </section>
				</section>
				<section style="margin-top: 1em; text-align: left;">
					...
					<section style="display: inline-block; width: 60%; padding: 0.2em; margin-left: 0.5em; font-size: 1em; color: inherit;" data-width="60%">活动地点：上海市普陀区交通路4703弄李子园商务区6号楼705、706室</section>
				</section>
				...
				<section style="padding: 1em; font-size: 1em; color: inherit;">人员：蒋婷，雪霁初晴，张小呆</section>
			</section>
        ...
        foreach ($json as $key => $val) {
            $html .= ($val['html'] != '') ? '<div class="hhtpldiv"><div class="hh-tpl-tpl">' . $val['html'] . '</div></div>' : '';
        }
        return $html;
    }
    ...
    public function downpic($url = '')
    {
        try {
            if ($url == '') return false;
            $path = ROOT_PATH. 'public/uploads/editor/';
            $date = date('Ymd');
            if (!is_dir($path . 'image'))  @mkdir($path . 'image', 0777, true);
            ...
            curl_close($ch);
            if ($httpinfo['http_code'] != 200)  return false;
    ...
    /**
     * base64转化为图片
     * @param unknown $base64str
     * @return number[]|string[]
     */
    public function base64ToImg($params = array()){
        $base64str = isset($params['base64str']) ? $params['base64str']:'';
        $folder   = (isset($params['folder']) && $params['folder'])? $params['folder']:date('Ymd');
        if(!$base64str) return array('success'=>0,'msg'=>'');
        if(preg_match('/^(data:\s*image\/(\w+);base64,)/',$base64str,$result)){
            $type = $result[2]; //jpeg
            $img = base64_decode(str_replace($result[1],'',$base64str)); //返回文件流
        }else{
            return array('success'=>0,'msg'=>'上传失败');
        }
        $rootPath = ROOT_PATH . 'public/uploads/';
        $subName  = $folder;
        $savepath = $rootPath.$subName;
        if (!file_exists($savepath)){mkdir($savepath);}
        $filename = date('YmdHis').rand(1000,9999).'.'.$type;
        file_put_contents($savepath.'/'.$filename, $img); //可以直接将文件流保存为本地图片
        return array('success'=>1,'msg'=>'上传成功','path'=>$subName.'/'.$filename,'picurl'=>cdnurl('/uploads/'.$subName.'/'.$filename));
    }
