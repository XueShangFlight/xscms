...
if (!function_exists('build_heading')) {
    ...
    function build_heading($path = null, $container = true)
    {
        ...
        $result = '<div class="panel-lead alert alert-tips"><em>' . $title . '</em>' . $content . '</div>';
        if ($container) {
...
if (!function_exists('icons')) {
     ...
     * 生成icon dom html
     */
    function icons($icon = '', $scene = '', $code = '', $class = '')
    {
        //fa-spin
        if ($icon == '') return '';
        $exarr = explode('_', $icon);
        if (count($exarr) == 2) {
            $scene = $exarr[1];
            $icon = $exarr[0];
        }
        $code = ($code == '') ? 'span' : $code;
        $iconclass = '';
        if ($scene == '') $iconclass = 'glyphicon glyphicon-' . $icon;
        if ($scene == 'fa') $iconclass = 'fa fa-' . $icon;
        if ($scene == 'pl') $iconclass = 'pli-' . $icon;
        if ($scene == 'net') $iconclass = $icon;
        $iconclass .= ($class != '') ? " {$class}" : '';
        return "<{$code} class='{$iconclass}'></{$code}>";
    }
}

if (!function_exists('tpltype')) {
    function tpltype()
    {
        $files = new \app\admin\controller\Files();
        return $files->gettpltype();
    }
}
